export const SHOWEYE = require('./show.png');
export const HIDEEYE = require('./hide.png');
export const LOGOBLACK = require('./logoblack.png');
export const LOGOWHITE = require('./logowhite.png');
export const IG = require('./instagram.png');
export const FB = require('./facebook.png');
export const GG = require('./google.png');
export const ARROW = require('./arrow-left.png');
export const ARROWW = require('./arroww.png');
export const ARROWD = require('./arrowdown.png');
export const LINE = require('./line.png');
export const SUCCESSFULLY = require('./successfully.jpg');
export const TICK = require('./tick.png');
export const BEACH = require('./beach.png');
export const MOUNTIAN = require('./mountain.png');
export const FOREST = require('./forest.png');
export const OCEAN = require('./ocean.png');
export const CAMPING = require('./camping.png');
export const FISHING = require('./fishing.png');
export const FLASHTWO = require('./flashtwo.jpg');
export const FLASHTHREE = require('./flashthree.jpg');
export const FLASHFOUR = require('./flashfour.jpg');
export const HEART = require('./heartred.png');
export const HEARTW = require('./heartwhite.png');
export const PICONE = require('./pic1.jpg');
export const TICKDETAIL = require('./tickdetail.png');
export const LOCATION = require('./location.png');
export const STAR = require('./star.png');
export const ONESTAR = require('./onestar.png');
export const TWOSTAR = require('./twostar.png');
export const THREESTAR = require('./threestar.png');
export const FOURSTAR = require('./fourstar.png');
export const FIVESTAR = require('./fivestar.png');
export const FLY = require('./fly.png');
export const HOTEL = require('./hotel.png');
export const CAR = require('./car.png');
export const UP = require('./up.png');
export const DOWN = require('./down.png');
export const METHODONE = require('./methodone.png');
export const PAYPAL = require('./paypal.png');
export const APPLE = require('./apple.png');
export const PLUS = require('./plus.png');
export const CARD = require('./card.jpg');
export const DOT = require('./dot.png');
export const DOTRED = require('./dotred.png');
export const MASTER = require('./mastercard.png');
export const BOOKING = require('./booking.jpg');
export const SEARCH = require('./search.png');
export const TICKET = require('./ticket.jpg');
export const CALENDAR = require('./calendar.png');
export const KUTARESORT = require('./kutaresort.jpg');
export const MANDALIKA = require('./mandalika.jpg');
export const PROFILE = require('./profile.png');
export const RING = require('./notification.png');
export const FAQ = require('./message.png');
export const LANGUAGE = require('./global.png');
export const NOTIONE = require('./notifione.png');
export const PROMO = require('./promo.png');
export const NOTITWO = require('./booking.png');
export const MAP = require('./map.jpg');
export const BROMO = require('./bromo.jpg');
export const FILTER = require('./filter.png');
export const DONE= require('./done.png');
export const RANGE= require('./range.png');











































